...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useEffect(() => {
    const geo = (nodes as any)[geometryKey]?.geometry;
    if (geo) {
  ...
  return (
    <>
      ...
      <mesh ref={ref} scale={scale ?? 0.15} rotation-x={Math.PI / 2} geometry={(nodes as any)[geometryKey]?.geometry} {...props}>
        <MeshTransmissionMaterial
...
function Images() {
  ...
  useFrame(() => {
    ...
    if (child0?.material) child0.material.zoom = 1 + data.range(0, 1 / 3) / 3;
    if (child1?.material) child1.material.zoom = 1 + data.range(0, 1 / 3) / 3;
    if (child2?.material) child2.material.zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
    if (child3?.material) child3.material.zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
    if (child4?.material) child4.material.zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
  });

  return (
    <group ref={group}>
      <Image position={[-2, 0, 0]} scale={3 as any} url="/assets/demo/cs1.webp" />
      <Image position={[2, 0, 3]} scale={3} url="/assets/demo/cs2.webp" />
      <Image position={[-2.05, -height, 6]} scale={3 as any} url="/assets/demo/cs3.webp" />
      <Image position={[-0.6, -height, 9]} scale={2 as any} url="/assets/demo/cs1.webp" />
